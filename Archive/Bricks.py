...
def create_category_df(categories, level, sheet):
    ...
    Category = pd.DataFrame(columns=sheet.drop(columns=['Segment Code', 'Family Code', 'Class Code']).columns)
...
def delete_category_df(categories, sheet):
    ...
    Delete_categories = sheet[sheet['Category Name'].isin(categories)]
...
def maxeda_sheet(sheet):
    ...
    maxeda_sheet = pd.read_excel(datamodel_file_path, sheet_name=sheet)
    # Select relevant columns
...
def sheet(sheet_df):
    ...
    for level, data in levels_and_data_new:
        new_categories = create_category_df(data, level, sheet_df)
...
# Map the descriptions to the 'Category Name' column using the combined dictionary
all_categories_s10_nl_NL['Mapped Description'] = all_categories_s10_nl_NL['Category Name'].map(code_to_description_dict_nl)
# Construct the new field with "[Category Name] - [Description]"
...
output_file_path = os.path.join(os.getcwd(), 'GS1_vs_Datamodel_Comparison.xlsx')
